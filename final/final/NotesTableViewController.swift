...
class FoodsTableViewController: UITableViewController {
    ...
    @IBAction func goBackToFoodsTableViewController(segue: UIStoryboardSegue) {
        if let controller = segue.source as? AddNoteTableViewController {
            notes.append(controller.Note)
            note.saveToFile(foods: notes)
            tableView.reloadData()
    ...
    override func viewDidLoad() {
        ...
        if let notes = note.readFoodsFromFile() {
            self.notes = notes
        }
    }
